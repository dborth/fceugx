...
uint8_t * DecodePNGToRGBA8(const uint8_t *src, int width, int height)
{
	...
	dst = static_cast<uint8_t *>(malloc(static_cast<uint32_t>(width) * height * 4));
	if(!dst)
	{
		png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
		return nullptr;
	...
	rowPointers = static_cast<png_bytep *>(malloc(sizeof(png_bytep) * height));
	if(!rowPointers)
	{
		free(dst);
		png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
	...
	else
	{
		...
		rowScratch = static_cast<uint8_t *>(malloc(static_cast<size_t>(rowBytes) * height));
		if(!rowScratch)
		{
			free(rowPointers);
			free(dst);
