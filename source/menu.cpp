 ...
 * user (when set to prompt) and capturing the RAM/state - is done right here
...
static int AutoSaveTask(void * arg)
{
	...
	if(job->ram)
		WriteRAMSnapshot(job->ram, SILENT);
	if(job->state)
		WriteStateSnapshot(job->state, SILENT);

	FreeRAMSnapshot(job->ram);
...
static bool AutoSave()
{
	...
	job->ram = saveRam ? SnapshotRAMAuto() : nullptr;
	job->state = saveState ? SnapshotStateAuto() : nullptr;

	if(!job->ram && !job->state)
	{
		delete job;
		return true;
	}
...
static int MenuSettingsVideo()
{
	...
	while(selection == MENU_NONE)
	{
		...
		if(ret >= 0 || firstRun)
		{
			...
			sprintf (options.value[8], "%.2f%%, %.2f%%", EmuSettings.videoZoomHor*100, EmuSettings.videoZoomVert*100);
			sprintf (options.value[9], "%d, %d", EmuSettings.videoXshift, EmuSettings.videoYshift);
...
static int MenuSettingsEmulation()
{
	...
	while(selection == MENU_NONE)
	{
		...
		switch (ret)
		{
			case 0:
				EmuSettings.timing++;
				if(EmuSettings.timing >= TIMING_LENGTH)
					EmuSettings.timing = TIMING_NTSC;
				break;
